...
export async function getGeoLocationByIPV4GO(licenseKey, isLive) {
  console.log("\n--------------------------------------------------------");
  console.log("IP Address Validation - GetGeoLocationByIP_V4 - REST SDK");
  console.log("--------------------------------------------------------");
  ...
  try {
    const response = await GetGeoLocationByIPV4Client.invoke(
      ipAddress,
      licenseKey,
      isLive,
      timeoutSeconds
